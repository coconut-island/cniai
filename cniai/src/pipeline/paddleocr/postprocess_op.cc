...
void DBPostProcessor::GetContourArea(const std::vector<std::vector<float>> &box,
    ...
    for (int i = 0; i < pts_num; i++) {
        area +=
            box[i][0] * box[(i + 1) % pts_num][1] - box[i][1] * box[(i + 1) % pts_num][0];
...
float DBPostProcessor::PolygonScoreAcc(std::vector<cv::Point> contour, cv::Mat pred) {
    ...
    int xmin = clamp(int(std::floor(*(std::min_element(box_x.begin(), box_x.end())))), 0,
                     width - 1);
    int xmax = clamp(int(std::ceil(*(std::max_element(box_x.begin(), box_x.end())))), 0,
                     width - 1);
    int ymin = clamp(int(std::floor(*(std::min_element(box_y.begin(), box_y.end())))), 0,
                     height - 1);
    int ymax = clamp(int(std::ceil(*(std::max_element(box_y.begin(), box_y.end())))), 0,
                     height - 1);
...
float DBPostProcessor::BoxScoreFast(std::vector<std::vector<float>> box_array,
    ...
    int xmin =
        clamp(int(std::floor(*(std::min_element(box_x, box_x + 4)))), 0, width - 1);
    int xmax = clamp(int(std::ceil(*(std::max_element(box_x, box_x + 4)))), 0, width - 1);
    int ymin =
        clamp(int(std::floor(*(std::min_element(box_y, box_y + 4)))), 0, height - 1);
    int ymax =
        clamp(int(std::ceil(*(std::max_element(box_y, box_y + 4)))), 0, height - 1);
    ...
    for (int _i = 0; _i < num_contours; _i++) {
        ...
        for (int num_pt = 0; num_pt < 4; num_pt++) {
            std::vector<int> a{int(clampf(roundf(cliparray[num_pt][0] / float(width) *
                                                 float(dest_width)),
                                          0, float(dest_width))),
                               int(clampf(roundf(cliparray[num_pt][1] / float(height) *
                                                 float(dest_height)),
                                          0, float(dest_height)))};
...
DBPostProcessor::FilterTagDetRes(std::vector<std::vector<std::vector<int>>> boxes,
...
void TablePostProcessor::Run(std::vector<float> &loc_preds,
                             std::vector<float> &structure_probs,
                             std::vector<float> &rec_scores,
                             std::vector<int> &loc_preds_shape,
...
void PicodetPostProcessor::init(std::string label_path, const double score_threshold,
...
void PicodetPostProcessor::Run(std::vector<StructurePredictResult> &results,
                               ...
                               std::vector<int> ori_shape, std::vector<int> resize_shape,
                               int reg_max) {
        ...
        for (int idx = 0; idx < feature_h * feature_w; idx++) {
            ...
            if (score > this->score_threshold_) {
                ...
                std::vector<float> bbox_pred(
                    ...
                    outs[i + this->fpn_stride_.size()].begin() + (idx + 1) * 4 * reg_max);
...
StructurePredictResult PicodetPostProcessor::disPred2Bbox(std::vector<float> bbox_pred,
