...
NvEncoderCuda::NvEncoderCuda(CUcontext cuContext, uint32_t nWidth, uint32_t nHeight,
...
void NvEncoderCuda::AllocateInputBuffers(int32_t numInputBuffers) {
    ...
    CU_CHECK(cuCtxPushCurrent(m_cuContext));
    std::vector<void *> inputFrames;
    for (int i = 0; i < numInputBuffers; i++) {
        CUdeviceptr pDeviceFrame;
        uint32_t chromaHeight = GetNumChromaPlanes(GetPixelFormat()) *
        ...
        CU_CHECK(cuMemAllocPitch((CUdeviceptr *)&pDeviceFrame, &m_cudaPitch,
...
void NvEncoderCuda::CopyToDeviceFrame(
    CUcontext device, void *pSrcFrame, uint32_t nSrcPitch, CUdeviceptr pDstFrame,
    uint32_t dstPitch, int width, int height, CUmemorytype srcMemoryType,
    NV_ENC_BUFFER_FORMAT pixelFormat, const uint32_t dstChromaOffsets[],
    uint32_t numChromaPlanes, bool bUnAlignedDeviceCopy, CUstream stream) {
    ...
    CU_CHECK(cuCtxPushCurrent(device));
    ...
    if (bUnAlignedDeviceCopy && srcMemoryType == CU_MEMORYTYPE_DEVICE) {
        CU_CHECK(cuMemcpy2DUnaligned(&m));
    } else {
        CU_CHECK(stream == NULL ? cuMemcpy2D(&m) : cuMemcpy2DAsync(&m, stream));
    }
