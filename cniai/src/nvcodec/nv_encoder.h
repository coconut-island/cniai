...
#define NVENC_THROW_ERROR(errorStr, errorCode)                                           \
    do {                                                                                 \
        throw NVENCException::makeNVENCException(errorStr, errorCode, __FUNCTION__,      \
                                                 __FILE__, __LINE__);                    \
    } while (0)
...
#define NVENC_API_CALL(nvencAPI)                                                         \
    do {                                                                                 \
        ...
        if (errorCode != NV_ENC_SUCCESS) {                                               \
            ...
            throw NVENCException::makeNVENCException(errorLog.str(), errorCode,          \
...
public:
    ...
    /**
     * @brief This function is used to get the current device type which encoder is
     * running.
     */
...
public:
    ...
    /**
     * @brief This a static function to get the number of chroma planes for YUV planar
     * formats.
     */
    static uint32_t GetNumChromaPlanes(const NV_ENC_BUFFER_FORMAT bufferFormat);

    /**
     * @brief This a static function to get the chroma plane width in bytes for YUV planar
     * formats.
     */
    ...
    /**
     * @brief This a static function to get the chroma planes height in bytes for YUV
     * planar formats.
     */
...
protected:
    ...
    /**
     * @brief This function is used to register CUDA, D3D or OpenGL input buffers with
     * NvEncodeAPI. This is non public function and is called by derived class for
     * allocating and registering input buffers.
     */
    ...
    /**
     * @brief This function is used to unregister resources which had been previously
     * registered for encoding using RegisterInputResources() function.
     */
    void UnregisterInputResources();

    /**
     * @brief This function is used to register CUDA, D3D or OpenGL input or output
     * buffers with NvEncodeAPI.
     */
    NV_ENC_REGISTERED_PTR
    RegisterResource(void *pBuffer, NV_ENC_INPUT_RESOURCE_TYPE eResourceType, int width,
                     int height, int pitch, NV_ENC_BUFFER_FORMAT bufferFormat,
     ...
     */
    // NVENCSTATUS DoMotionEstimation(NV_ENC_INPUT_PTR inputBuffer, NV_ENC_INPUT_PTR
    // inputBufferForReference, NV_ENC_OUTPUT_PTR outputBuffer);
...
private:
    ...
    */
    ...
    /**
     * @brief This is a private function which is used to load the encode api shared
     * library.
     */
    void LoadNvEncApi();
    ...
    void GetEncodedPacket(std::vector<NV_ENC_OUTPUT_PTR> &vOutputBuffer,
                          ...
                          std::vector<uint64_t> &outputTimeStamps, bool bOutputDelay);

    /**
     * @brief This is a private function which is used to initialize the bitstream
...
private:
    ...
    uint32_t m_nExtraOutputDelay =
        3; // To ensure encode and graphics can work in parallel, m_nExtraOutputDelay
