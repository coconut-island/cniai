...
void NvEncoder::LoadNvEncApi() {
    ...
    if (currentVersion > version) {
        NVENC_THROW_ERROR("Current Driver Version does not support this NvEncodeAPI "
                          "version, please upgrade driver",
...
void NvEncoder::CreateDefaultEncoderParams(NV_ENC_INITIALIZE_PARAMS *pIntializeParams,
...
#endif
    m_nvenc.nvEncGetEncodePresetConfigEx(m_hEncoder, codecGuid, presetGuid, tuningInfo,
                                         &presetConfig);
    ...
    if (pIntializeParams->encodeGUID == NV_ENC_CODEC_H264_GUID) {
        if (m_eBufferFormat == NV_ENC_BUFFER_FORMAT_YUV444 ||
            ...
            pIntializeParams->encodeConfig->encodeCodecConfig.h264Config.chromaFormatIDC =
                3;
        }
        ...
        if (m_eBufferFormat == NV_ENC_BUFFER_FORMAT_YUV444 ||
            m_eBufferFormat == NV_ENC_BUFFER_FORMAT_YUV444_10BIT) {
            pIntializeParams->encodeConfig->encodeCodecConfig.hevcConfig.chromaFormatIDC =
                3;
        }
...
void NvEncoder::CreateEncoder(const NV_ENC_INITIALIZE_PARAMS *pEncoderParams) {
    ...
    if (pEncoderParams->encodeGUID == NV_ENC_CODEC_H264_GUID) {
        if ((m_eBufferFormat == NV_ENC_BUFFER_FORMAT_YUV444) &&
            (pEncoderParams->encodeConfig->encodeCodecConfig.h264Config.chromaFormatIDC !=
             3)) {
    ...
    if (pEncoderParams->encodeGUID == NV_ENC_CODEC_HEVC_GUID) {
...
#if (NVENCAPI_MAJOR_VERSION >= 12 && NVENCAPI_MINOR_VERSION >= 2)
        if (yuv10BitFormat &&
            pEncoderParams->encodeConfig->encodeCodecConfig.hevcConfig.inputBitDepth !=
                NV_ENC_BIT_DEPTH_10)
#else
        if (yuv10BitFormat && pEncoderParams->encodeConfig->encodeCodecConfig.hevcConfig
                                      .pixelBitDepthMinus8 != 2)
#endif
        {
        ...
        if ((m_eBufferFormat == NV_ENC_BUFFER_FORMAT_YUV444 ||
             m_eBufferFormat == NV_ENC_BUFFER_FORMAT_YUV444_10BIT) &&
            (pEncoderParams->encodeConfig->encodeCodecConfig.hevcConfig.chromaFormatIDC !=
             3)) {
            NVENC_THROW_ERROR("Invalid ChromaFormatIDC", NV_ENC_ERR_INVALID_PARAM);
        }
    }
    ...
    if (pEncoderParams->encodeConfig) {
        ...
        NV_ENC_PRESET_CONFIG presetConfig = {NV_ENC_PRESET_CONFIG_VER,
                                             {NV_ENC_CONFIG_VER}};
#endif
        m_nvenc.nvEncGetEncodePresetConfigEx(m_hEncoder, pEncoderParams->encodeGUID,
                                             pEncoderParams->presetGUID,
    ...
    m_nEncoderBuffer = m_encodeConfig.frameIntervalP +
                       m_encodeConfig.rcParams.lookaheadDepth + m_nExtraOutputDelay;
...
void NvEncoder::GetEncodedPacket(std::vector<NV_ENC_OUTPUT_PTR> &vOutputBuffer,
    ...
    for (; m_iGot < iEnd; m_iGot++) {
        ...
        NVENC_API_CALL(
            m_nvenc.nvEncUnlockBitstream(m_hEncoder, lockBitstreamData.outputBitstream));
...
NV_ENC_REGISTERED_PTR NvEncoder::RegisterResource(
    void *pBuffer, NV_ENC_INPUT_RESOURCE_TYPE eResourceType, int width, int height,
    int pitch, NV_ENC_BUFFER_FORMAT bufferFormat, NV_ENC_BUFFER_USAGE bufferUsage,
