...
public:
    ...
    virtual int width() const override { return x1 - x0; }

    virtual int height() const override { return y1 - y0; }

    virtual int x_offset_on_bitmap() const override { return offset_x; }
    ...
    StbWordMeta(int x0, int y0, int x1, int y1, float scale, int advance, int glyph,
                int offset_x) {
        this->x0 = x0;
        this->y0 = y0;
        this->x1 = x1;
...
class StbWordMetaMapperImpl : public WordMetaMapper,
...
static bool file_exist(const string &path) { return access(path.c_str(), R_OK) == 0; }

static vector<tuple<string, string>>
find_files(const string &directory, const string &suffix, bool includeSubDirectory) {
            ...
            while (true) {
                ...
                if (!S_ISDIR(file_stat.st_mode)) {
                    if (suffix_match(fileinfo->d_name))
                        out.push_back(
                            make_tuple(search_path + fileinfo->d_name, fileinfo->d_name));
                }
...
static string get_ttf_path_from_family_name(const char *_font_family) {
    ...
    std::transform(font_family.begin(), font_family.end(), font_family.begin(),
                   ::tolower);
    ...
    for (size_t imatch = 0; imatch < match_list.size(); ++imatch) {
        ...
        for (auto file : files) {
            ...
            lower_name = raw_name;
            std::transform(raw_name.begin(), raw_name.end(), lower_name.begin(),
                           ::tolower);
...
public:
    ...
    measure_text(const std::vector<unsigned long int> &words, unsigned int font_size,
        ...
        for (auto &word : words) {
            ...
            if (iter == word_map.end()) {
                ...
                stbtt_GetGlyphBitmapBoxSubpixel(pfont, glyph, scale, scale, 0, 0, &x0,
                                                &y0, &x1, &y1);
    ...
    virtual void add_build_text(const std::vector<unsigned long int> &words,
                                unsigned int font_size, const char *font) override {
    ...
    virtual void build_bitmap(void *_stream) override {
        ...
        for (auto &textes : build_use_textes) {
            ...
            for (auto &word : words) {
                ...
                stbtt_GetGlyphBitmapBoxSubpixel(pfont, glyph, scale, scale, 0, 0, &x0,
                                                &y0, &x1, &y1);
                glyph_map.insert(make_pair(
                    word, StbWordMeta(x0, y0, x1, y1, scale, advance, glyph, 0)));
            }

            const char *default_words = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuv"
                                        "wxyz0123456789:-&./&^%$#@!+=\\[];,'\"?` ";
        ...
        for (auto &map : this->glyph_sets) {
            for (auto &item : map.second) {
                int w = item.second.x1 - item.second.x0,
                    h = item.second.y1 - item.second.y0;
        ...
        for (auto &map : this->glyph_sets) {
            ...
            for (auto &item : map.second) {
                ...
                stbtt_Rasterize(&gbm, 0.35f, vertices, num_verts, glyph.scale,
                                glyph.scale, 0, 0, glyph.x0, glyph.y0, 1,
                                pfont->userdata);
    ...
    virtual int bitmap_width() const override { return this->text_bitmap_width; }
    ...
    virtual int uniform_font_size(int size) const override { return size * 3; }
};
