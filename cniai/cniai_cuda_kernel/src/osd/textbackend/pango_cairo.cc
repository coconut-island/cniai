...
class PangoWordMetaMapperImpl : public WordMetaMapper,
                                public map<unsigned long int, PangoWordMeta> {
public:
...
public:
    PangoCairoBackend() {
        ...
        this->single_word_surface = cairo_image_surface_create_for_data(
            this->single_word_bitmap->host(), CAIRO_FORMAT_A8, temp_size, temp_size,
            temp_size);
        ...
        for (auto &word : words) {
            ...
            if (iter == word_map.end()) {
                PangoFontDescription *desc =
                    pango_font_description_from_string(font_and_size.c_str());
    ...
    virtual void build_bitmap(void *_stream) override {
        ...
        for (auto &textes : build_use_textes) {
            ...
            for (auto &word : words) {
                ...
                if (!desc_set) {
                    desc_set = true;
                    PangoFontDescription *desc =
                        pango_font_description_from_string(textes.first.c_str());
                    pango_layout_set_font_description(this->layout, desc);
                    pango_font_description_free(desc);
                }
        ...
        for (auto &map : this->glyph_sets) {
            PangoFontDescription *desc =
                pango_font_description_from_string(map.first.c_str());
            pango_layout_set_font_description(this->layout, desc);
            pango_font_description_free(desc);
            ...
            for (auto &item : map.second) {
                ...
                memset(this->single_word_bitmap->host(), 0,
                       this->single_word_bitmap->bytes());
                ...
                for (int iy = 0; iy < h;
                     ++iy, prow += this->text_bitmap_width, psrc += this->temp_size) {
                    memcpy(prow, psrc, w);
                }
                offset_x += w;
            }
    ...
    virtual int uniform_font_size(int size) const override { return size; }
};
