...
#include "cuosd_memory.h"
#include "textbackend/backend.h"
...
struct TextHostCommand : cuOSDContextCommand {
    ...
    TextHostCommand(const vector<unsigned long int> &text, unsigned short font_size,
                    const char *font, int x, int y, unsigned char c0, unsigned char c1,
...
void cuosd_measure_text(cuOSDContext_t _context, const char *utf8_text, int font_size,
    ...
    if (context->text_backend == nullptr)
        context->text_backend =
            create_text_backend(convert_to_text_backend_type(context->text_backend_type));
...
void cuosd_draw_text(cuOSDContext_t _context, const char *utf8_text, int font_size,
    ...
    int width, height, yoffset;
    tie(width, height, yoffset) = context->text_backend->measure_text(
...
void cuosd_draw_clock(cuOSDContext_t _context, cuOSDClockFormat clock_format, long time,
    ...
    cuosd_draw_text(_context, utf8_str.c_str(), font_size, font, x, y, border_color,
                    bg_color);
}
...
void cuosd_draw_boxblur(cuOSDContext_t _context, int left, int top, int right, int bottom,
                        int kernel_size) {
    ...
    if (right - left < 3 || bottom - top < 3 || kernel_size < 1) {
        CUOSD_PRINT_E("[cuosd_draw_boxblur] This operation will be ignored because the "
...
void cuosd_draw_rotationbox(cuOSDContext_t _context, int cx, int cy, int width,
                            ...
                            bool interpolation, cuOSDColor bg_color) {
    cuOSDContextImpl *context = (cuOSDContextImpl *)_context;
    if (border_color.a == 0)
        return;
    ...
    cmd->interpolation = interpolation;
    cmd->ax1 =
        cos_angle * (pax - half_thickness) - sin_angle * (pay - half_thickness) + cx;
    cmd->ay1 =
        sin_angle * (pax - half_thickness) + cos_angle * (pay - half_thickness) + cy;
    cmd->bx1 =
        cos_angle * (pbx - half_thickness) - sin_angle * (pby + half_thickness) + cx;
    cmd->by1 =
        sin_angle * (pbx - half_thickness) + cos_angle * (pby + half_thickness) + cy;
    cmd->cx1 =
        cos_angle * (pcx + half_thickness) - sin_angle * (pcy + half_thickness) + cx;
    cmd->cy1 =
        sin_angle * (pcx + half_thickness) + cos_angle * (pcy + half_thickness) + cy;
    cmd->dx1 =
        cos_angle * (pdx + half_thickness) - sin_angle * (pdy - half_thickness) + cx;
    cmd->dy1 =
        sin_angle * (pdx + half_thickness) + cos_angle * (pdy - half_thickness) + cy;
...
void cuosd_draw_polyline(cuOSDContext_t _context, int *h_pts, int *d_pts, int n_pts,
                         int thickness, bool is_closed, cuOSDColor border_color,
                         bool interpolation, cuOSDColor fill_color) {
    cuOSDContextImpl *context = (cuOSDContextImpl *)_context;
    ...
    for (int i = 1; i < n_pts; i++) {
        ...
        cuosd_draw_line(context, h_pts[2 * i - 2], h_pts[2 * i - 1], x, y, thickness,
                        border_color, interpolation);
        nline++;
    }
    ...
    if (n_pts > 2) {
        if (is_closed) {
            cuosd_draw_line(context, h_pts[0], h_pts[1], h_pts[2 * n_pts - 2],
                            h_pts[2 * n_pts - 1], thickness, border_color, interpolation);
            nline++;
        }
...
void cuosd_draw_circle(cuOSDContext_t _context, int cx, int cy, int radius, int thickness,
                       cuOSDColor border_color, cuOSDColor bg_color) {
    cuOSDContextImpl *context = (cuOSDContextImpl *)_context;
    if (bg_color.a && thickness > 0)
        context->commands.emplace_back(make_shared<CircleCommand>(
            cx, cy, radius, -thickness, bg_color.r, bg_color.g, bg_color.b, bg_color.a));
    context->commands.emplace_back(
        make_shared<CircleCommand>(cx, cy, radius, thickness, border_color.r,
...
void cuosd_draw_nv12_source(cuOSDContext_t _context, int left, int top, int right,
                            int bottom, void *d_src0, void *d_src1, int src_width,
                            int src_stride, int src_height, unsigned char alpha,
                            bool block_linear) {
    cuOSDContextImpl *context = (cuOSDContextImpl *)_context;
    int tl = min(left, right);
    int tt = min(top, bottom);
...
void cuosd_apply(cuOSDContext_t _context, void *data0, void *data1, int width, int stride,
                 int height, cuOSDImageFormat format, void *_stream,
    ...
    if (!context->blur_commands.empty()) {
        ...
        for (int i = 0; i < (int)context->blur_commands.size(); ++i) {
            ...
            memcpy((void *)(context->gpu_blur_commands->host() + i), (void *)cmd.get(),
                   sizeof(BoxBlurCommand));
        }
...
void cuosd_launch(cuOSDContext_t _context, void *data0, void *data1, int width,
                  int stride, int height, cuOSDImageFormat format, void *stream) {
    cuOSDContextImpl *context = (cuOSDContextImpl *)_context;
    if (context->commands.empty() && context->blur_commands.empty()) {
        CUOSD_PRINT_W("Please check if there is anything to draw\n");
        return;
    ...
    cuosd_launch_kernel(
        ...
        context->text_location ? context->text_location->device() : nullptr, text_bitmap,
        text_bitmap_width,
        ...
        context->bounding_right, context->bounding_bottom, context->have_rotate_msaa,
